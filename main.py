...
async def upload_file(file: UploadFile = File(...)):
    ...
    return {"info": f"文件 '{file.filename}' 上传成功，存储在 '{file_location}'."}
...
async def list_files(request: Request):
    ...
    files = os.listdir(UPLOAD_DIRECTORY)
    return templates.TemplateResponse("download_by_code.html", {"request": request, "files": files})
...
@app.get("/downloadfile/{pickup_code}")
